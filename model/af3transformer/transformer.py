...
class PointTransformerAF3(torch.nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.linear_3d_embed = torch.nn.Linear(3*28, kwargs["token_dim"], bias=False)
    ...
    def _extend_w_registers(self, seqs, pair, mask, cond_seq):
        ...
        Returns:
            All elements above extended with regist`ers / zeros.
        """
    ...
    def forward(self, batch_nn: Dict[str, torch.Tensor]):
        ...
        Args:
            batch_nn: dictionary with keys
                - "x_t": tensor of shape [b, n_residue, 3*28]
                - "t": tensor of shape [b]
        ...
        coors_3d = batch_nn["x_t"] * mask[..., None]  # [b, n, 3*28]
        coors_embed = (
            self.linear_3d_embed(coors_3d) * mask[..., None]
        )  # [b, n, token_dim]
        seq_f_repr = batch_nn['seq_fea']  # [b, n, token_dim]
