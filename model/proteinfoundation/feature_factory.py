...
from utils.ff_utils.pdb_utils import extract_cath_code_by_level
from utils.ff_utils.idx_emb_utils import get_index_embedding, get_time_embedding
...
class XscSeqFeat(Feature):
    ...
    def __init__(self, **kwargs):
        super().__init__(dim=kwargs["token_dim"])
...
# class XtPairwiseDistLocalCoordsPairFeat(Feature):
...
#         """
#         xt = batch["x_t"]  # [b, n, 28*3]
#         center = (xt.reshape(xt.shape[0], xt.shape[1], 28, 3).sum(dim=-1) 
#                   / batch["atom_mask"].sum(dim=-1)[..., None])  # [b, n, 3]
#         center = torch.cumsum(center, dim=2) # From Local Corrds to Global Corrds
...
class FeatureFactory(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.ln_out = (
            torch.nn.LayerNorm(dim_feats_out, eps=1e-5) if use_ln_out else torch.nn.Identity()
        )
    ...
    def get_creator(self, f, **kwargs):
        ...
        if self.mode == "seq":
            ...
            # elif f == "xt_pair_dists_from_local_coords":
            #     return XtPairwiseDistLocalCoordsPairFeat(**kwargs)
