...
from model.proteinfoundation.feature_factory import FeatureFactory
from model.proteinfoundation.pair_bias_attn.pair_bias_attn import PairBiasAttention
from model.proteinfoundation.alphafold3_pytorch_utils.modules import (
    AdaptiveLayerNorm,
...
class GatedFusion(nn.Module):
    ...
    def forward(self, seq_feats: torch.Tensor, struct_feats: torch.Tensor) -> torch.Tensor:
        """
        Args:
            seq_feats:   [B, N, D] 序列特征 (如BERT输出)
            struct_feats:[B, N, D] 结构特征 (最后3维为坐标)
        Returns:
            fused_feats: [B, N, hidden_dim] 融合特征
        """
        
        # 交叉注意力交互 (网页7的多模态交互)
        attn_seq, _ = self.cross_attn(
            query=seq_feats,
            key=struct_feats,
            value=struct_feats
        )
        ...
        # 门控融合 (整合网页6和网页8)
        combined = torch.cat([attn_seq, struct_feats], dim=-1)
        combined = self.proj(combined)  # [B, N, H]
...
class PointTransformerAF3(torch.nn.Module):
    """
    ...
    def __init__(self, **kwargs):
        ...
        super(PointTransformerAF3, self).__init__()
        ...
        self.linear_3d_embed_rna = nn.Sequential(
            nn.Linear(28*3, kwargs["token_dim"]),
            nn.LayerNorm(kwargs["token_dim"]),
            nn.GELU(),
            nn.Linear(kwargs["token_dim"], kwargs["token_dim"]),
        )
        ...
        self.fea_fusion_fn = GatedFusion(hidden_dim=kwargs["token_dim"])
        ...
        self.coors_3d_decoder_rna = torch.nn.Sequential(
            torch.nn.LayerNorm(kwargs["token_dim"]),
            torch.nn.Linear(kwargs["token_dim"], 3 * 28, bias=False),
        )
    ...
    def forward(self, batch_nn: Dict[str, torch.Tensor]):
        ...
        Args:
            batch_nn: dictionary with keys
                - "x_t": tensor of shape [b, n, 28*3]
                - "t": tensor of shape [b]
                - "mask": binary tensor of shape [b, n]
                - "atom_mask": binary tensor of shape [b, n, 28]
                - "x_sc" (optional): tensor of shape [b, n, 3]
                - "cath_code" (optional): list of cath codes [b, ?]
                - And potentially others... All in the data batch.

        Returns:
            Predicted clean coordinates, shape [b, n, 28*3].
        """
        mask = batch_nn["mask"]
        atom_mask = batch_nn['atom_mask']
        B, N, D = batch_nn["x_t"].shape
        ...
        coors_3d = batch_nn["x_t"].view(B, N, -1, 3) * atom_mask[..., None] # [b, n, 28*3]
        coors_3d = coors_3d.view(B, N, -1)  # [b, n, 28*3]
        
        coors_embed = (
            self.linear_3d_embed_rna(coors_3d) * mask[..., None]
        )  # [b, n, token_dim]
        seq_f_repr = self.init_repr_factory(batch_nn)  # [b, n, token_dim]
        # seqs = coors_embed + seq_f_repr  # [b, n, token_dim]
        seqs = self.fea_fusion_fn(seq_f_repr, coors_embed)
        ...
        final_coors = self.coors_3d_decoder_rna(seqs) * mask[..., None]  # [b, n, 3]
        final_coors = final_coors.view(B, N, -1, 3) * atom_mask[..., None]
        final_coors = final_coors.view(B, N, -1) + batch_nn["x_t"]
        # nn_out = {}
        # if self.update_pair_repr and self.num_buckets_predict_pair is not None:
        #     # pair_pred = self.pair_head_prediction(pair_rep)
        #     final_coors = (
        #         final_coors + torch.mean(pair_rep) * 0.0
        #     )  # Does not affect loss but pytorch does not complain for unused params
        #     final_coors = final_coors.view(B, N, -1, 3) * atom_mask[..., None]
        #     final_coors = final_coors.view(B, N, -1) * 0.00001 + batch_nn["x_t"]
        #     # nn_out["pair_pred"] = pair_pred
        # nn_out["coors_pred"] = final_coors
        return final_coors.reshape(B, -1, 3)